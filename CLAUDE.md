...
| `npm run zones:stats` | Recalcule les stats PME réelles par ville (API recherche-entreprises.api.gouv.fr) → `data/zones/stats.json`. À relancer tous les 3 mois. |
| `npm run zones:sync` | Synchronise `data/zones/stats.json` vers la base Notion "Zones d'intervention" (tableau de pilotage, ne pilote pas le contenu des pages) |
...
**31 août 2026** — Pages SEO locales programmatiques `/zones/[slug]/` (9 villes, vague 1) : Le Mans, Sablé-sur-Sarthe, La Ferté-Bernard, La Flèche, Mamers, Château-du-Loir (Sarthe) + Laval, Alençon, Angers (limitrophes). Hors menu Header (uniquement lien discret Footer + hub `/zones/`), indexables via sitemap. Contenu différencié par ville pour éviter le duplicate content : stats PME réelles (API recherche-entreprises.api.gouv.fr, `scripts/fetch_zone_stats.mjs` → `data/zones/stats.json`, à rafraîchir tous les 3 mois), secteur NAF dominant local, FAQ locale, éditorial dans `src/data/zones.ts` (contrôle qualité manuel, pas généré). Schema.org `Service` + `areaServed` (pas `LocalBusiness` avec adresse — Alliance Digitale = Service Area Business). Base Notion "Zones d'intervention" (`NOTION_ZONES_DB_ID`) créée comme tableau de pilotage (stats + statut), ne contrôle pas le contenu des pages qui reste dans le repo. Prochaine étape : mesurer indexation/impressions GSC à J+60 avant vague 2 (Tier 3 : Saint-Calais, Écommoy, etc. — cf. spec seo-local complète dans l'historique de session).

**17 août 2026** — Mise à jour outil Sécurité IA (`/securite-ia/`) : données modèles actualisées dans `src/data/securite-ia/models.fr.json` (Claude = entraînement chat par défaut depuis sept. 2025, opt-out, rétention 5 ans, API exclue ; labels GPT-5.x, Gemini 3.x, Opus 5, Grok 4.5, Kimi K3, Mistral 3/Medium 3.5 ; note rétention judiciaire OpenAI). Ajout champ `last_verified` (source unique) affiché « Données vérifiées le JJ/MM/AAAA » dans le disclaimer des 3 gabarits (`index.astro`, `[sector].astro`, `[doc].astro`). Lexique : GPT-4 → GPT-5. Commit `ecd09a2`, déployé prod Vercel, vérifié sur www.alliance-digitale.fr. À la prochaine révision des données : changer `last_verified`.
